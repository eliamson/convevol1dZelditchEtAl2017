#conevol3 for univariate analysis 
# find vector of ancestors
pullNodeSeq1d <- function(phy, t1)	{
	...
	if (Anc != Stop)		{
		Anc <- c(Anc, pullNodeSeq1d(phy, Anc))
	}
	return(as.character(Anc))
}
# calc distance between ancestors of two tips and their ancestors
calcCs1d <- function(tips, ancList, allDists, phy, VERBOSE=FALSE)	{
	if (VERBOSE == TRUE)		{
		cat("Analzying ", tips)
	}
	# tip distances
...
# workhorse function
calcConv1d <- function(phy, traits, focaltaxa, VERBOSE=FALSE)	{
	require(phytools)
	if (class(phy) != "phylo")	{
	...
	#Originally was here: traits <- traits[phy$tip.label,]
	if (VERBOSE == TRUE)		{
		print("reached traits")
	}
	# for C1: need phylogeny, trait matrix, ancestral recos
	ACE <- fastAnc(tree=phy,traits) #ORI: ACE <- apply(traits, 2, fastAnc, tree=phy)
	...
	Cmat <- apply(Combinations, 2, function(x) calcCs1d(x, ancList=Ancs, allDists=allDists, phy=phy, VERBOSE=VERBOSE))

	out <- apply(Cmat,1,mean)
	return(out)
}

convSig1DZelditch <- function(phy, traits, focaltaxa, nsim=1e3)	{
	data <- calcConv1d(phy, traits, focaltaxa)
	
	# run simulations
	phylMat <- vcv.phylo(phy)
	phylMat2 <- phyl.vcv(cbind(traits), phylMat, 0)
	simDat <- sim.char(phy, phylMat2$R, nsim, model="BM", root=0)
	simOut <- apply(simDat, 3, calcConv1d, phy=phy, focaltaxa=focaltaxa)
